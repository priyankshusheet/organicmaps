...
namespace {
NSString *const kAdditionalNamesEditorSegue =
    @"Editor2AdditionalNamesEditorSegue";
...
NSString *const kUDEditorPersonalInfoWarninWasShown =
    @"PersonalInfoWarningAlertWasShown";
...
std::map<MWMEditorCellID, Class> const kCellType2Class{
    ...
    {MWMEditorCellTypeAdditionalName,
     [MWMEditorAdditionalNameTableViewCell class]},
    {MWMEditorCellTypeAddAdditionalName,
     [MWMEditorAddAdditionalNameTableViewCell class]},
...
void cleanupAdditionalLanguages(
    std::vector<osm::LocalizedName> const &names,
...
std::vector<MWMEditorCellID>
cellsForAdditionalNames(osm::NamesDataSource const &ds,
  ...
  if (allNamesSize != 0) {
    if (showAdditionalNames) {
      ...
      res.insert(res.begin(), mandatoryNamesCount,
                 MWMEditorCellTypeAdditionalName);
...
void registerCellsForTableView(std::vector<MWMEditorCellID> const &cells,
                               UITableView *tv) {
...
@interface MWMEditorViewController () <
    ...
    MWMOpeningHoursEditorProtocol, MWMPlacePageOpeningHoursCellProtocol,
    MWMEditorCellProtocol, MWMCuisineEditorProtocol, MWMStreetEditorProtocol,
    MWMObjectsCategorySelectorDelegate, MWMNoteCellDelegate,
    MWMEditorAdditionalName, MWMButtonCellDelegate,
    MWMEditorAdditionalNamesProtocol>

@property(nonatomic)
    NSMutableDictionary<Class, UITableViewCell *> *offscreenCells;
...
- (void)viewDidLoad {
  ...
  self.featureStatus =
      osm::Editor::Instance().GetFeatureStatus(fid.m_mwmId, fid.m_index);
  self.isFeatureUploaded =
      osm::Editor::Instance().IsFeatureUploaded(fid.m_mwmId, fid.m_index);
...
- (void)setEditableMapObject:(osm::EditableMapObject const &)emo {
  NSAssert(self.isCreating,
           @"We should pass featureID to editor if we just editing");
  m_mapObject = emo;
}
...
- (void)configNavBar {
  self.title = L(self.isCreating ? @"editor_add_place_title"
...
- (void)onSave {
  if (![self.view endEditing:YES]) {
    NSAssert(false, @"We can't save map object because one of text fields "
                    @"can't apply it's text!");
    return;
  }
  ...
  if (haveNote)
    f.CreateNote(m_mapObject, osm::Editor::NoteProblemType::General,
                 self.note.UTF8String);
...
- (void)setShowAdditionalNames:(BOOL)showAdditionalNames {
  ...
  [self configTable];
  auto const additionalNamesSectionIt = find(
      m_sections.begin(), m_sections.end(), MWMEditorSectionAdditionalNames);
  if (additionalNamesSectionIt == m_sections.end()) {
    [self.tableView reloadData];
  } else {
    auto const sectionIndex =
        distance(m_sections.begin(), additionalNamesSectionIt);
    [self.tableView
          reloadSections:[[NSIndexSet alloc] initWithIndex:sectionIndex]
...
- (UITableViewCell *)offscreenCellForClass:(Class)cls {
  ...
  if (!cell) {
    cell = [NSBundle.mainBundle loadWithViewClass:cls owner:nil options:nil]
               .firstObject;
...
- (void)configTable {
  ...
  editableProperties.erase(
      std::remove_if(editableProperties.begin(), editableProperties.end(),
                     [](osm::MapObject::MetadataID mid) {
                       return mid == MetadataID::FMD_POSTCODE ||
                              mid == MetadataID::FMD_BUILDING_LEVELS;
                     }),
  ...
  if (isNameEditable) {
    ...
    auto const cells = cellsForAdditionalNames(ds, m_newAdditionalLanguages,
                                               self.showAdditionalNames);
    ...
    [self.additionalNamesHeader
        setAdditionalNamesVisible:cells.size() > ds.mandatoryNamesCount + 1];
  }
  ...
  if (isAddressEditable) {
    ...
    if (m_mapObject.IsBuilding() && !m_mapObject.IsPointType())
      m_cells[MWMEditorSectionAddress].push_back(
...
- (void)configTextViewCell:(UITableViewCell *_Nonnull)cell
  ...
  MWMEditorTextTableViewCell *tCell =
      static_cast<MWMEditorTextTableViewCell *>(cell);
  [tCell configWithDelegate:self
                       icon:[UIImage imageNamed:icon]
                       text:ToNSString(m_mapObject.GetMetadata(
                                static_cast<MetadataID>(cellID)))
                placeholder:name
               errorMessage:error
                    isValid:isValid
...
- (void)fillCell:(UITableViewCell *_Nonnull)cell
  ...
  case MWMEditorCellTypeCategory: {
    ...
    auto const bookmarkIcon = GetBookmarkIconByFeatureType(type);
    NSString *iconName = [NSString
        stringWithFormat:@"ic_bm_%@", [@(kml::ToString(bookmarkIcon).c_str())
                                          lowercaseString]];
    [cCell configureWithDelegate:self
                            icon:[UIImage imageNamed:iconName]
                     detailTitle:@(platform::GetLocalizedTypeName(readableType)
                                       .c_str())
  ...
  case MetadataID::FMD_OPEN_HOURS: {
    MWMPlacePageOpeningHoursCell *tCell =
        static_cast<MWMPlacePageOpeningHoursCell *>(cell);
    NSString *text = ToNSString(m_mapObject.GetOpeningHours());
    [tCell configWithDelegate:self
                         info:(text.length ? text : L(@"add_opening_hours"))];
    break;
  }
  case MetadataID::FMD_INTERNET: {
    MWMEditorSwitchTableViewCell *tCell =
        static_cast<MWMEditorSwitchTableViewCell *>(cell);
    ...
    [tCell configWithDelegate:self
                         ...
                         text:L(@"category_wifi")
                           on:m_mapObject.GetInternet() ==
                              feature::Internet::Wlan];
    break;
  }
  case MWMEditorCellTypeAdditionalName: {
    ...
    if (indexPath.row < localizedNames.size()) {
      ...
      NSString *langName = indexPath.row == StringUtf8Multilang::kDefaultCode
                               ? L(@"editor_default_language_hint")
      ...
      NSInteger const newAdditionalNameIndex =
          indexPath.row - localizedNames.size();
      NSInteger const langCode =
          m_newAdditionalLanguages[newAdditionalNameIndex];
      ...
      [tCell
          configWithDelegate:self
                    langCode:langCode
                    langName:ToNSString(StringUtf8Multilang::GetLangNameByCode(
                                 langCode))
                        name:@(name.c_str())
                errorMessage:L(@"error_enter_correct_name")
                     isValid:isValid
  ...
  case MWMEditorCellTypeAddAdditionalNamePlaceholder:
    break;
  case MWMEditorCellTypeStreet: {
    MWMEditorSelectTableViewCell *tCell =
        static_cast<MWMEditorSelectTableViewCell *>(cell);
    [tCell configWithDelegate:self
  ...
  case MetadataID::FMD_BUILDING_LEVELS: {
    NSString *placeholder = [NSString
        stringWithFormat:L(@"editor_storey_number"),
                         osm::EditableMapObject::kMaximumLevelsEditableByUsers];
    NSString *errorMessage = [NSString
        stringWithFormat:L(@"error_enter_correct_storey_number"),
                         osm::EditableMapObject::kMaximumLevelsEditableByUsers];
    [self configTextViewCell:cell
                      cellID:cellID
                        icon:nil
  ...
  case MetadataID::FMD_DRIVE_THROUGH: {
    MWMEditorSegmentedTableViewCell *tCell =
        static_cast<MWMEditorSegmentedTableViewCell *>(cell);
    [tCell configWithDelegate:self
                         icon:[UIImage imageNamed:@"ic_placepage_drive_through"]
                         text:L(@"drive_through")
                        value:feature::YesNoUnknownFromString(
                                  m_mapObject.GetMetadata(
                                      feature::Metadata::FMD_DRIVE_THROUGH))];
    break;
  }
  case MetadataID::FMD_SELF_SERVICE: {
    ...
    [tCell configWithDelegate:self
                        ...
                        value:feature::YesNoUnknownFromString(
                                  m_mapObject.GetMetadata(
                                      feature::Metadata::FMD_SELF_SERVICE))];
    break;
  }
  case MetadataID::FMD_OUTDOOR_SEATING: {
    ...
    [tCell
        configWithDelegate:self
                     ...
                     value:feature::YesNoUnknownFromString(
                               m_mapObject.GetMetadata(
                                   feature::Metadata::FMD_OUTDOOR_SEATING))];
    break;
  }
  ...
  case MWMEditorCellTypeReportButton: {
    ...
    auto title = ^NSString *(FeatureStatus s, BOOL isUploaded) {
      ...
      case FeatureStatus::Obsolete: // TODO(Vlad): Either make a valid button or
                                    // disable it.
    ...
    [tCell
        configureWithDelegate:self
                        title:title(self.featureStatus, self.isFeatureUploaded)
                      enabled:YES];
    break;
  }
  default:
...
- (CGFloat)tableView:(UITableView *_Nonnull)tableView
  ...
  default: {
    ...
    [cell setNeedsLayout];
    [cell layoutIfNeeded];
    CGSize const size = [cell.contentView
...
- (UIView *)tableView:(UITableView *)tableView
  ...
  case MWMEditorSectionButton:
    return nil;
  case MWMEditorSectionDetails:
    if (find(m_sections.begin(), m_sections.end(), MWMEditorSectionNote) ==
        m_sections.end())
...
- (CGFloat)tableView:(UITableView *)tableView
  ...
  return m_sections[section] == MWMEditorSectionNote ? kDefaultHeaderHeight * 2
...
- (void)editAdditionalNameLanguage:(NSInteger)selectedLangCode {
  [self performSegueWithIdentifier:kAdditionalNamesEditorSegue
                            sender:@(selectedLangCode)];
}
...
- (void)addAdditionalName:(NSInteger)languageIndex {
  ...
  auto additionalNamesSectionIt = find(m_sections.begin(), m_sections.end(),
                                       MWMEditorSectionAdditionalNames);
  assert(additionalNamesSectionIt != m_sections.end());
  auto const section = distance(m_sections.begin(), additionalNamesSectionIt);
  NSInteger const row =
      [self tableView:self.tableView numberOfRowsInSection:section];
  assert(row > 0);
...
- (void)cell:(MWMTableViewCell *)cell changedText:(NSString *)changeText {
  ...
  case MWMEditorCellTypeAdditionalName:
    ...
    if (isFieldValid)
      m_mapObject.SetName(
          val, static_cast<MWMEditorAdditionalNameTableViewCell *>(cell).code);
    break;
  default:
    auto const metadataID = static_cast<MetadataID>(cellType);
    ASSERT_LESS(metadataID, MetadataID::FMD_COUNT, ());
    isFieldValid =
        osm::EditableMapObject::IsValidMetadata(metadataID, val) ? YES : NO;
...
- (void)cell:(UITableViewCell *)cell changeSwitch:(BOOL)changeSwitch {
  ...
  switch ([self cellTypeForIndexPath:indexPath]) {
  case MetadataID::FMD_INTERNET:
    m_mapObject.SetInternet(changeSwitch ? feature::Internet::Wlan
                                         : feature::Internet::Unknown);
    break;
  default:
...
- (void)cell:(UITableViewCell *)cell
    changeSegmented:(YesNoUnknown)changeSegmented {
  NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
  switch ([self cellTypeForIndexPath:indexPath]) {
  case MetadataID::FMD_DRIVE_THROUGH:
  ...
  default:
    NSAssert(false, @"Invalid field for changeSegmented");
    break;
  }
}
...
- (void)tapOnButtonCell:(UITableViewCell *)cell {
  ...
  auto placeDoesntExistAction = ^{
    [self.alertController
        presentPlaceDoesntExistAlertWithBlock:^(NSString *additionalMessage) {
          std::string const additional =
              additionalMessage.length ? additionalMessage.UTF8String : "";
          GetFramework().CreateNote(
              self->m_mapObject,
              osm::Editor::NoteProblemType::PlaceDoesNotExist, additional);
          [self goBack];
          [self showNotesQueuedToast];
        }];
  };
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  if ([segue.identifier isEqualToString:kOpeningHoursEditorSegue]) {
    ...
    MWMEditorAdditionalNamesTableViewController *dvc =
